...
def test_tempo_positive(genre_config):
...
def test_tempo_plausible_range(genre_config):
    """L1-02: tempo within 40..200."""
    _, _, tempo = layer_1_rhetorical(genre_config=genre_config)
    assert 40 <= tempo <= 200, f"Tempo {tempo} outside plausible range"


def test_rhythm_vocab_nonempty(genre_config):
...
def test_rhythmic_unit_valid_fraction(genre_config):
    """L1-04: rhythmic_unit parseable as Fraction."""
    _, rhythm_vocab, _ = layer_1_rhetorical(genre_config=genre_config)
    assert "rhythmic_unit" in rhythm_vocab
    unit = Fraction(rhythm_vocab["rhythmic_unit"])
    assert unit > 0


def test_trajectory_length(genre_config):
...
def test_trajectory_matches_sections(genre_config):
