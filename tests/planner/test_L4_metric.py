...
) -> tuple[dict[str, tuple[int, int]], tuple[Anchor, ...], int, GenreConfig, KeyConfig]:
...
def test_bar_assignments_complete(
    metric_result: tuple[dict[str, tuple[int, int]], tuple[Anchor, ...], int, GenreConfig, KeyConfig],
) -> None:
...
def test_bar_assignments_contiguous(
    metric_result: tuple[dict[str, tuple[int, int]], tuple[Anchor, ...], int, GenreConfig, KeyConfig],
) -> None:
    """L4-02: bar assignments are contiguous with no gaps."""
    bar_assignments, _, _, genre_config, _ = metric_result
    section_names: list[str] = [s["name"] for s in genre_config.sections]
    ranges: list[tuple[int, int]] = [bar_assignments[name] for name in section_names]
    ranges_sorted: list[tuple[int, int]] = sorted(ranges, key=lambda r: r[0])
