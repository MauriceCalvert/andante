...
from builder.types import SchemaChain
...
# TODO: L3-11, L3-12 deferred until redesign stabilises


def _section_ranges(boundaries):
...
def test_boundary_count(schema_result):
...
def test_boundaries_monotonic(schema_result):
...
def test_cadence_entries_valid(schema_result):
...
def test_one_cadence_per_section(schema_result):
    """L3-14: each section has exactly one cadence at its last schema."""
    chain, _, _ = schema_result
    ranges = _section_ranges(chain.section_boundaries)
