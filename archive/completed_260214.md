...
## IMP-2 — Subject Planner (2026-02-15)

Built `plan_subject()` in `planner/imitative/subject_planner.py` that reads
`entry_sequence` from genre YAML and produces a complete SubjectPlan with one
BarAssignment per bar. No musical output change — galant path still runs.

**Changes (3 modified, 1 new):**
- NEW `planner/imitative/subject_planner.py`: `plan_subject()` walks entry_sequence
  top-to-bottom, creates VoiceAssignment per voice per bar, assigns sections
  proportionally, validates full coverage. Guards against unsupported entry types
  (episode/stretto/pedal assert with clear message).
- `shared/constants.py`: Added `CADENCE_BARS = 1` constant.
- `planner/planner.py`: At imitative branch, calls `plan_subject()` when
  thematic_config and fugue are present. Logs SubjectPlan summary (total_bars,
  entry_count, section names). Imported `plan_subject`.
- `scripts/yaml_validator.py`: Added `composition_model` to `VALID_GENRE_KEYS`.

**Verification:**
- SubjectPlan: 13 bars (6 entries x 2 bars + 1 cadence x 1 bar)
- Bars 1-13 all assigned, no gaps or overlaps
- Key resolution: I=C major, V=G major, vi=A minor, IV=F major
- Sections: exordium (1-4), narratio (5-8), confirmatio (9-12), peroratio (13)
- All 8 genres pass run_tests with identical output
- Invention note counts unchanged (322 soprano, 105 bass at seed 42)

## IMP-1 — Imitative Path Infrastructure (2026-02-15)

Added branching infrastructure for the imitative composition path.
No musical change — invention still produces output via the galant
path (the branch logs a fallback warning).

Changes:
- `builder/types.py`: Added `composition_model: str = "galant"` to GenreConfig
- `builder/config_loader.py`: Parses `composition_model` from genre YAML,
  validates "galant" or "imitative", defaults to "galant"
- `data/genres/invention.yaml`: Added `composition_model: imitative`
- `planner/imitative/__init__.py`: Empty (L018)
- `planner/imitative/types.py`: VoiceAssignment, BarAssignment, SubjectPlan
  frozen dataclasses (data definitions only, no logic)
- `planner/planner.py`: Branch after L2 checks composition_model=="imitative",
  logs warning, falls through to galant path

Verification: All 8 genres pass run_tests with identical output.
Invention trace shows "Imitative path not yet implemented" fallback.

## TD-1t — Thematic Trace Instrumentation (2026-02-14)

Added trace instrumentation to the thematic system. No musical changes.

### Code Changes

**`shared/tracer.py`:**
- Added `trace_thematic_plan()` — L4b bar table with role coverage
- Added `trace_thematic_render()` — L6t per-entry render dispatch
- Added `trace_thematic_coverage()` — L6t coverage summary with FREE gap analysis
- Added `_collapse_bar_roles()` helper to format bar roles compactly

**`planner/thematic.py`:**
- Added entry sequence echo: `L4b Entries: S.I|--- -> cs.I|A.V -> ...`
- Added `_format_entry_sequence_echo()` helper function

**`planner/planner.py`:**
- Initialize `thematic_plan = None` before conditional (scope fix)
- Call `tracer.trace_thematic_plan()` after `plan_thematic_roles()` returns
- Call `tracer.trace_thematic_coverage()` after `compose_phrases()` returns

**`builder/phrase_writer.py`:**
- Added trace calls in `_write_thematic()` material_entries loop
- Trace emitted per entry per voice (SUBJECT/ANSWER/CS/EPISODE/FREE)

### Trace Output Format

Invention trace now shows:
```
L4b Entries: S.I|--- -> cs.I|A.V -> S.vi|cs.vi -> cs.IV|S.IV -> S.V|cs.V -> cs.I|S.I -> CAD
L4b Thematic: 7 entries parsed, 23 bars, 44 thematic (95.7%)
  Roles: ANSWER=8 CADENCE=88 COUNTERSUBJECT=40 FREE=8 SUBJECT=40
  Bar  Upper               Lower
    1  SUBJECT  C maj      ---
    2  SUBJECT  C maj      ---
    3  COUNTERSUBJECT C maj ANSWER   G maj
    ...
L6t Render: bar  1 U SUBJECT C maj   -> 10 notes C4..A4
L6t Render: bar  1 L ---
L6t Render: bar  3 U COUNTERSUBJECT C maj   ->  3 notes B4..E5
L6t Render: bar  3 L ANSWER  G maj   ->  3 notes D3..A3
...
L6t Coverage: 176/184 beats thematic (95.7%) | FREE 8
```

### Checkpoint Results

✅ All checkpoints passed:
- Invention trace contains L4b Thematic, L4b Entries, L6t Render, L6t Coverage sections
- Galant genres (gavotte) show no L4b/L6t sections
- No trace output when -trace flag absent
- Full test suite passes (8 genres)
- Zero musical output change

### Trace Points

Trace calls sit on stable output interfaces:
1. Entry sequence echo — after parsing, before placement
2. Thematic plan table — after `plan_thematic_roles()` returns
3. Render dispatch — per entry in `_write_thematic()` loop
4. Coverage summary — after `compose_phrases()` returns

Future thematic work (episode fragmentation, stretto, etc.) benefits automatically.
