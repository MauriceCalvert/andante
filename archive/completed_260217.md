...
## SUBJECT-GEN-REFACTOR — Modular package structure (2026-02-19)

Refactored `motifs/subject_generator.py` (953 lines) into focused package
`motifs/subject_gen/` with 10 modules. Single-file architecture split into
clean dependency graph with clear separation of concerns.

**New package structure:**
- `models.py` — GeneratedSubject, _ScoredPitch dataclasses (35 lines)
- `constants.py` — All configuration constants + _bar_x2_ticks (55 lines)
- `cache.py` — Disk cache utilities with corrected path for nesting (30 lines)
- `validator.py` — Melodic validity checks (25 lines)
- `scoring.py` — Generic scoring utilities (20 lines)
- `contour.py` — Pitch contour analysis + shape classification (115 lines)
- `pitch_generator.py` — Stage 1 exhaustive pitch generation + scoring (240 lines)
- `duration_generator.py` — Stage 2 bar-fill duration enumeration + scoring (170 lines)
- `selector.py` — Diversity selection + subject assembly (200 lines)
- `__init__.py` — Public API surface (3 exports)

**Top-level wrapper:**
- `subject_generator.py` reduced to 81-line CLI wrapper (was 953 lines)
- Imports from package, keeps display helpers + argparse logic

**External consumers updated:**
- `answer_generator.py` → imports from subject_gen (2 locations)
- `writers.py` → imports from subject_gen + subject_gen.constants (3 locations)
- `countersubject_generator.py` → imports from subject_gen (2 locations)
- `stretto_analyser.py` → imports from subject_gen (1 location)
- `simple_subject.py` → partial update (file was already broken)

**Cache compatibility verified:** _CACHE_DIR navigates up 3 levels (was 2) to
maintain same `.cache/subject/` location. All imports successful. CLI works via
`python -m motifs.subject_generator`.

**Benefits:** 90% line reduction in top-level file, clear module boundaries,
reusable components (cache, scoring, contour), easier testing, no circular
dependencies. Follows L002 (constants centralized), L015 (dataclasses), D008
(clean architecture).
