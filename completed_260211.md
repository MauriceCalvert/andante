...
## 2026-02-11 — Phase 16g: DiminutionFill Cross-Bar Repetition and Leap-Step Prevention

Added three counterpoint checks to `_check_figure_pitches` in `builder/strategies/diminution.py`:
1. Cross-bar repetition (D007) — rejects figures where non-structural pitches repeat across bar boundaries
2. Leap-step recovery — rejects figures where leaps lack contrary stepwise resolution
3. Consecutive leaps — rejects figures with three consecutive same-direction leaps

All checks use shared functions from `shared.counterpoint` (L017 compliance). Implementation added 3 new imports (has_consecutive_leaps, is_cross_bar_repetition, needs_step_recovery) and 5 new if statements (3 counterpoint checks + 2 guards) to the per-pitch checking loop.

**Test results:**
- Overall: 3734 passed, 33 failed (baseline 3730/37) — 4-test improvement, zero new failures
- Leap-step recovery: 48/49 pass ✓ (meets acceptance criteria ≤2 failures)
- Cross-bar repetition: 18/49 pass (31 failures — does NOT meet acceptance criteria ≤5)

**Cross-bar analysis:** The 31 failures originate from `_stepwise_fallback`, which calls placeholder `select_best_pitch` that does not check cross-bar repetition. Task Known Limitations #2 explicitly identifies this and defers to Phase 16h. `_check_figure_pitches` correctly rejects violating figures, but increased rejection rate → more stepwise fallback usage → more violations from placeholder logic.

**Chaz verdict:** Code structure correct. All 6 constraints met: 3 new imports, 3 new checks using shared functions, `_stepwise_fallback` unchanged. No regressions. Leap-step prevention highly effective. Cross-bar failures documented and deferred per specification.
...
## 2026-02-11 — Phase 16h: Constraint Scoring for select_best_pitch

Implemented full constraint scoring in `shared/pitch_selection.py` per task specification. Replaced placeholder `select_best_pitch` (returned first candidate) with penalty-based scoring using power-of-2 weights for strict priority ordering.

**Implementation:**
- `select_best_pitch`: iterates candidates, scores each with `_score_pitch`, returns lowest penalty
- `_score_pitch`: checks 7 constraints (priority 7→1), returns weighted sum
- Penalties: 1 (step recovery), 2 (consecutive leaps), 4 (ugly intervals), 8 (cross-bar rep), 16 (cross-relations), 32 (parallel perfects), 64 (voice crossing)
- All checks use shared functions from `shared/counterpoint` (L017 compliance)
- Linter refactored to boolean arithmetic instead of if/else cascade (more concise)

**Test results:**
- Overall: 3734 passed, 33 failed (Phase 16g baseline: 3734/33) — **zero change**
- Cross-bar repetition: 18/49 pass (31 failures) — **zero improvement**
- Leap-step recovery: 48/49 pass (1 failure) — **zero improvement**

**Critical finding:** Task assumptions incorrect. Failures occur in **figure path**, not fallback path:
- Debug confirmed `_stepwise_fallback` NOT called at failure offsets (2.9375→3.0)
- All 31 cross-bar repetitions have `is_structural=True` (one offset structural)
- `is_cross_bar_repetition` **exempts structural offsets by design** (line 238 counterpoint.py)
- Figures pass `_check_figure_pitches` (respects exemption) but fail test (no exemption)
- `select_best_pitch` never invoked for failing cases (figures used, not fallback)

**Root cause:** Architectural — figure-generated notes repeat structural tone pitches across bars. Constraint allows (structural exempt), test forbids (no exempt). Fixing requires modifying constraint design or figure selection logic, both outside Phase 16h scope (task forbids modifying counterpoint.py and diminution.py).

**Chaz verdict:** Implementation correct. Signature unchanged ✓. All 7 constraints checked ✓. Power-of-2 weights ✓. Empty other_voices handling ✓. Zero if statements (boolean arithmetic) ✓. Code quality excellent. Test results reveal task premise was incorrect, not implementation defect.

**Status:** Accepted per user choice (option 1 — implementation correct per spec, task assumptions wrong).
